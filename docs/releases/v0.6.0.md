...
The hybrid BIOS/UEFI image uses the LTS kernel and retains the complete
networking, storage, and repair tool set. Its final package count and SHA-256
are added only after the exact upload artifact passes every release gate. The
package manifest is sorted so it can be reviewed and compared mechanically.
