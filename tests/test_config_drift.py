...
def test_resolv_conf_packaged_comments_only_is_handled_autonomously(tmp_path):
    root = tmp_path / "etc"
    root.mkdir()
    target = root / "resolv.conf"
    drift = root / "resolv.conf.pacnew"
    target.write_text("# Generated by NetworkManager\nnameserver 127.0.0.53\n", encoding="utf-8")
    drift.write_text("# Resolver configuration file.\n# See resolv.conf(5) for details.\n", encoding="utf-8")

    action = plan_config_drift_action(classify_config_drift_file(drift, root=root))

    assert action.action == "preserve_active_remove_packaged_comments"
    assert action.applies is True
    assert action.requires_confirmation is True
    assert action.remove_drift is True


def test_pacman_conf_cachyos_policy_merges_safe_packaged_defaults(tmp_path):
    root = tmp_path / "etc"
    root.mkdir()
    target = root / "pacman.conf"
    drift = root / "pacman.conf.pacnew"
    target.write_text(
        "[options]\n"
        "Color\n"
        "#CheckSpace\n"
    ...
    assert action.action == "merge_pacman_conf_safe_options"
    assert action.applies is True
    assert action.requires_confirmation is True
    assert "CheckSpace\n" in action.candidate_text
    assert "#CheckSpace" not in action.candidate_text
    assert "ParallelDownloads = 10" in action.candidate_text
    assert "ParallelDownloads = 5" not in action.candidate_text
    assert "[cachyos-v3]" in action.candidate_text
    assert "[multilib]" in action.candidate_text
...
def test_apply_preserve_active_action_backs_up_and_removes_pacnew(tmp_path):
    root = tmp_path / "etc"
    root.mkdir()
    target = root / "resolv.conf"
    drift = root / "resolv.conf.pacnew"
    target.write_text("nameserver 127.0.0.53\n", encoding="utf-8")
    drift.write_text("# packaged comments only\n", encoding="utf-8")
    report = build_config_drift_report(root)

    ok = apply_config_drift_actions(report, backup_root=tmp_path / "backups")

    assert ok is True
    assert target.read_text(encoding="utf-8") == "nameserver 127.0.0.53\n"
    assert not drift.exists()
    assert report.applied[0]["action"] == "preserve_active_remove_packaged_comments"
...
def test_terminal_hides_disabled_ai_diff_review_noise(tmp_path):
    root = tmp_path / "etc"
    root.mkdir()
    (root / "mirrorlist").write_text("old\n", encoding="utf-8")
...
def test_config_drift_cli_prompt_defaults_yes_for_fully_planned_fixes(tmp_path):
    root = tmp_path / "etc"
    root.mkdir()
    target = root / "mirrorlist"
    drift = root / "mirrorlist.pacnew"
    ...
    assert status == 0
    assert prompts == ["AuraScan prepared config drift fixes. Apply now? [Y/n] "]
    assert target.read_text(encoding="utf-8") == "new\n"
    assert not drift.exists()


def test_config_drift_cli_prompt_explicit_no_declines_default_yes(tmp_path):
    root = tmp_path / "etc"
    root.mkdir()
    target = root / "mirrorlist"
    drift = root / "mirrorlist.pacnew"
...
def test_config_drift_cli_prompt_defaults_no_when_manual_review_remains(tmp_path):
    root = tmp_path / "etc"
    root.mkdir()
    target = root / "mirrorlist"
    drift = root / "mirrorlist.pacnew"
    target.write_text("old\n", encoding="utf-8")
    drift.write_text("new\n", encoding="utf-8")
    manual = root / "binary.conf.pacnew"
    manual.write_bytes(b"a\x00b")
    prompts = []
    ...
    declined = run_config_drift(
        ...
        stdout=io.StringIO(),
        stderr=io.StringIO(),
        backup_root=tmp_path / "backups-manual",
    )
...
def test_config_drift_cli_yes_applies_without_prompt(tmp_path):
    root = tmp_path / "etc"
    root.mkdir()
    target = root / "mirrorlist"
    drift = root / "mirrorlist.pacnew"
