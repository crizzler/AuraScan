...
def test_current_release_recovery_manifest_policy_is_versioned_and_fresh():
    ...
    date_label = "Release date:" if manifest["status"] == "build-required" else "Released on"
    date_pattern = r"^" + re.escape(date_label) + r" ([0-9]{4}-[0-9]{2}-[0-9]{2})\.$"
    match = re.search(date_pattern, release, re.MULTILINE)
...
def test_pyproject_console_scripts_are_registered():
    ...
    assert data["project"]["version"] == "0.10.7"
...
def test_recovery_bearing_v0104_release_contract():
    release = " ".join(read_text("docs/releases/v0.10.4.md").lower().split())
    required_release_phrases = (
        "# aurascan v0.10.4",
        "released on 2026-09-06",
        "recovery-bearing release",
        "`aurascan-recovery-0.10.4-x86_64.iso`",
        ...
        "package-scanner rule version advances to `1.6.0`",
        "previous release or tag is rewritten",
        "application and arch/aur package advance to v0.10.4",
    )
    for phrase in required_release_phrases:
        assert phrase in release
    assert "v0.10.4 release refreshes the optional hybrid bios/uefi" in " ".join(
        read_text("docs/ANNOUNCEMENT.md").lower().split()
    )


def test_recovery_bearing_v0105_release_contract():
    release = " ".join(read_text("docs/releases/v0.10.5.md").lower().split())
    required_release_phrases = (
        "# aurascan v0.10.5",
        "released on 2026-09-09",
        "recovery-bearing release",
        "`aurascan-recovery-0.10.5-x86_64.iso`",
        "sorted `.iso.packages.txt` manifest",
        "rejects a github release asset at or above 2 gib",
        "fresh work and output locations",
        "either `recovery-bearing` or `package-only`",
        "locally built ukis remain specific",
        "package-scanner rule version advances to `1.6.1`",
        "previous release or tag is rewritten",
        "application and arch/aur package advance to v0.10.5",
    )
    for phrase in required_release_phrases:
        assert phrase in release
    announcement = " ".join(read_text("docs/ANNOUNCEMENT.md").lower().split())
    assert "v0.10.5 release" in announcement
    assert "adds bounded review of codewhale and legacy deepseek" in announcement


def test_recovery_bearing_v0106_release_contract():
    release = " ".join(read_text("docs/releases/v0.10.6.md").lower().split())
    required_release_phrases = (
        "# aurascan v0.10.6",
        "released on 2026-09-09",
        "recovery-bearing release",
        "`aurascan-recovery-0.10.6-x86_64.iso`",
        "sorted `.iso.packages.txt` manifest",
        "rejects a github release asset at or above 2 gib",
        "fresh work and output locations",
        "either `recovery-bearing` or `package-only`",
        "locally built ukis remain specific",
        "package-scanner rule version advances to `1.7.0`",
        "previous release or tag is rewritten",
        "application and arch/aur package advance to v0.10.6",
    )
    for phrase in required_release_phrases:
        assert phrase in release
    announcement = " ".join(read_text("docs/ANNOUNCEMENT.md").lower().split())
    assert "v0.10.6 release" in announcement
    assert "adds source-attributed shai-hulud npm" in announcement


def test_recovery_bearing_v0107_release_contract():
    import json

    def normalized(relative: str) -> str:
        return " ".join(read_text(relative).lower().split())

    release_text = read_text("docs/releases/v0.10.7.md")
    ...
    required_release_phrases = [
        "# aurascan v0.10.7",
        "release date: 2026-09-11" if manifest["status"] == "build-required" else "released on 2026-09-11",
        "recovery-bearing release",
        ...
        "either `recovery-bearing` or `package-only`",
        "locally built ukis remain specific",
        "package-scanner rule version advances to `1.9.0`",
        "previous release or tag is rewritten",
        "application and arch/aur package advance to v0.10.7",
    ]
    for phrase in required_release_phrases:
        assert phrase in release

    assert "changes after v0.10.7" in unreleased
    assert "release disposition" in checklist
    assert "strictly smaller than 2 gib (2,147,483,648 bytes)" in checklist
    assert "v0.10.7 release" in announcement
    assert "adds bounded editor-task inspection" in announcement
    assert "pkgver=0.10.7" in pkgbuild
    assert "pkgrel=1" in pkgbuild
    assert "sha256sums=('SKIP')" in pkgbuild or re.search(r"sha256sums=\('[0-9a-f]{64}'\)", pkgbuild)
    assert "\tpkgver = 0.10.7" in srcinfo
    assert "aurascan-0.10.7.tar.gz" in srcinfo
    assert "\tsha256sums = SKIP" in srcinfo or re.search(r"\tsha256sums = [0-9a-f]{64}", srcinfo)
    assert 'return "0.10.7-dev"' in recovery_source
    ...
    assert manifest["application_version"] == "0.10.7"
    assert manifest["release_disposition"] == "recovery-bearing"
    assert manifest["version"] == "0.10.7"
    assert manifest["architecture"] == "x86_64"
    assert manifest["filename"] == "aurascan-recovery-0.10.7-x86_64.iso"
    assert manifest["released_at"] == "2026-09-11"
    ...
    if manifest["status"] == "build-required":
        ...
        assert manifest["url"].endswith(
            "/v0.10.7/aurascan-recovery-0.10.7-x86_64.iso"
        )
        assert re.fullmatch(r"[0-9a-f]{64}", manifest["sha256"])

    if os.environ.get("AURASCAN_RELEASE_FINAL") == "1":
        assert os.environ.get("AURASCAN_RELEASE_TAG") == "v0.10.7"
        assert manifest["status"] == "release-ready"
        assert re.fullmatch(r"[0-9a-f]{64}", manifest["sha256"])
